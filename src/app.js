...
var fs = require("fs");
var nanotimer_1 = require("nanotimer");
...
gpio_1["default"].init(shutdown, handleClockSwitch, handleResetIn, handleClock);
midi_1["default"].init(handleMidiNoteOn, handleMidiCCmessage);
display_1["default"].init();
keyboard_1["default"].init();
sequencer_1["default"].init();
presets = loadPresets();
var timer = new nanotimer_1["default"]();
function loadPresets() {
    var fileData = fs.readFileSync(__dirname + "/presets.json", "utf8");
...
function handleClock(err, value) {
    ...
    console.log("Midi clock @ " + midi_1["default"].bpm);
    // these only get updated once per beat
    keyboard_1["default"].upate();
    sequencer_1["default"].update();
    // trigger external clock if running internally
    if (midi_1["default"].internalClock) {
        fireExternalClock();
    }
}
...
function update() {
    ...
    midi_1["default"].update();
    display_1["default"].update();
}
// start updates @ 30 fpx
timer.setInterval(update, '33m');
